...
DOX_chr_main <- load("DOX_noDOX.RData")
DOX_chr_main <- sort(subset(gr_combined, seqnames == "19"))
...
collapsed_df <- readRDS("collapsed_df_chr19.rds")
...
DOX_chr19_main_df <- as.data.frame(DOX_chr19_main)
highlighted_regions <- geom_rect(data = DOX_chr19_main_df, aes(xmin = start, xmax = end, ymin = -Inf, ymax = Inf), 
                                 fill = "blue", alpha = 0.3)

# Raw 7mer mapping plot
p.simplepeaks <- ggplot(mapped_7mer_df, aes(xmin = start, xmax = end, ymin = 0, ymax = enrichment, fill = enrichment)) + 
...
p.eclip <- ggplot(TARDBP.sites.on.UNC13A, aes(xmin = start, xmax = end, ymin = 0, ymax = log2FoldEnrichment, fill = log2FoldEnrichment)) + 
  geom_rect(show.legend = TRUE) +
  theme_bw() +
  ylab("") +
  scale_x_continuous(labels = comma) +
...
# taxonomic model of UNC13A
p.txmodels <- plotTXmodelOverview(UNC13A.txmodels)
...
plt <- cowplot::plot_grid(
  ...
  p.simplepeaks + coord_cartesian(xlim = c(xmin, xmax)),
...
# TDB43 simple peaks
p.CHR_simple <- ggplot(Chr19_simplePeaks_df, aes(xmin = start, xmax = end, ymin = 0, ymax = enrichment, fill = enrichment)) + 
  geom_rect(show.legend = FALSE) +
  highlighted_regions +
  theme_bw() +
  ylab("") +
...
legend_plot <- ggplot(mapped_7mer_df, aes(xmin = start, xmax = end, ymin = 0, ymax = enrichment, fill = enrichment)) + 
  geom_rect() +
  scale_fill_distiller(palette = 'Spectral', limits = c(0, 2.4)) +
  guides(fill = guide_colorbar(title = "Enrichment")) +
  theme_void() +
...
# ggsave("HDGFL2_zoom.png", plot = zoom, width = 10, height = 4)
# ggsave("UNC13A_zoom.png", plot = zoom, width = 10, height = 4)
