...
### CELF5 - eCLIP and Taxonomic model ###
CELF5.txmodels <- extractTXmodelsFromAH(gene.of.interest = "CELF5")
CELF5.gr <- genes(ensdb, filter =~ gene_name == "CELF5")

overlaps <- findOverlaps(GRanges(TARDBP.eclip), CELF5.gr, type = "within")

TARDBP.sites.on.CELF5 <- TARDBP.eclip[queryHits(overlaps),]
...
p.eclip <- ggplot(TARDBP.sites.on.CELF5, aes(xmin = start, xmax = end, ymin = 0, ymax = log2FoldEnrichment, fill = log2FoldEnrichment)) + 
  geom_rect(show.legend = TRUE) +
  theme_bw() +
  ylab("") +
  scale_x_continuous(labels = comma) +
